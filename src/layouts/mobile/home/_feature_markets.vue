<template>
  <swiper :options="swiperOption" class="feature-markets">
    <swiper-slide>
      <li
        v-for="(data, index) in featureMarkets('one')"
        :key="`one` + index"
        class="market"
        @click="openMarketPreview(data.split('/'))"
      >
        <p class="name">{{ data.replace("/", " / ") }}</p>
        <p class="last">{{ getLastPrice(data) }}</p>
        <p :class="[getTrend(percentToNumber(getChange(data))), 'rate']">
          {{ getChange(data) }}
        </p>
      </li>
    </swiper-slide>
    <swiper-slide>
      <li
        v-for="(data, index) in featureMarkets('two')"
        :key="`two` + index"
        class="market"
        @click="openMarketPreview(data.split('/'))"
      >
...
export default {
  ...
  methods: {
    ...
    getSparkline(ticker) {
      return this.$store.getters["public/getAllSparkline"][
