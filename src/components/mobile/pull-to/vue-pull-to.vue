<template>
  ...
  >
    <div
      ...
      :style="{
        height: `${topBlockHeight}px`,
        marginTop: `${-(
          topBlockHeight - (['loading', 'loaded-done'].includes(state) ? 5 : 0)
        )}px`
      }"
    ...
    >
      ...
      >
        ...
        <p class="last-update">last update: {{ getLastUpdateTime }}</p>
      </slot>
    </div>
    <div ref="scroll-container" class="scroll-container">
      <slot></slot>
      <div
        v-if="bottomLoadMethod && isBottomKeepScroll"
        ref="bottom-filler"
        class="bottom-filler"
      ></div>
    </div>
    <div
      ...
      :style="{
        ...
        marginBottom: `${-bottomBlockHeight}px`
      }"
...
export default {
  ...
  computed: {
    ...
    getLastUpdateTime() {
      ...
      return helpers.getDate(lastUpdate);
    }
  },
  watch: {
    state(val) {
      ...
      if (typeof val !== "string" || val === "") {
        ...
        this.scrollTo(0, 300, c.loadedStayTime);
      }
    },
...
.vue-pull-to-wrapper > .action-block > p {
  text-align: center;
  line-height: 20px;
